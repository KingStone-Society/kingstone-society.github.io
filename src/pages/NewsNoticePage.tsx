...
const NewsNoticePage: React.FC = () => {
  ...
  const filteredArticles = articles.filter(article => 
    article.section.includes('金石资讯') && article.section.includes('公告公示')
  );
      ...
      <div className="w-full lg:w-[200px]">
        <div className="bg-white border border-xlys-beige-dark p-4">
          ...
          <ul className="flex flex-wrap gap-2 lg:block lg:space-y-2">
            ...
            <li className="flex-1 lg:flex-none">
              <Link to="/news/notice" className="block bg-xlys-red text-white px-3 py-2 text-sm text-center">公告公示</Link>
            </li>
          </ul>
        </div>
      </div>
      <div className="flex-1">
        <div className="bg-white border border-xlys-beige-dark p-4">
          <div className="flex items-center text-xs text-xlys-gray mb-6">
            ...
            <span> &gt; </span>
            <span className="text-xlys-red">公告公示</span>
          </div>
          <h2 className="text-xlys-dark font-bold text-xl mb-6 border-b border-xlys-beige-dark pb-2">公告公示</h2>
          {filteredArticles.length > 0 ? (
            ...
            <div className="text-center text-xlys-gray py-10">
              ...
              <p className="text-xs mt-2">并在文章头部设置 section: ["金石资讯", "公告公示"]</p>
            </div>
          )}
        </div>
      </div>
