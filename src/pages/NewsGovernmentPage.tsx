...
const NewsGovernmentPage: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const filteredArticles = articles.filter(article => 
    article.section.includes('金石资讯') && article.section.includes('政务信息')
  );
      ...
      <div className="flex-1">
        <div className="bg-white border border-xlys-beige-dark p-4">
          <div className="flex items-center text-xs text-xlys-gray mb-6">
            ...
            <span> &gt; </span>
            <span className="text-xlys-red">政务信息</span>
          </div>
          <h2 className="text-xlys-dark font-bold text-xl mb-6 border-b border-xlys-beige-dark pb-2">政务信息</h2>
          {filteredArticles.length > 0 ? (
            ...
            <div className="text-center text-xlys-gray py-10">
              ...
              <p className="text-xs mt-2">并在文章头部设置 section: ["金石资讯", "政务信息"]</p>
            </div>
          )}
        </div>
      </div>
...
export default NewsGovernmentPage;
